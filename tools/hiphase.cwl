...
outputs:
  phased_deepvariant_vcf: { type: 'File', outputBinding: { glob: '*deepvariant*.vcf.gz' }, secondaryFiles: ['.tbi'] }
  phased_pbsv_vcf: { type: 'File', outputBinding: { glob: '*pbsv*.vcf.gz' }, secondaryFiles: ['.tbi'] }
  ...
  haplotag_file_out: { type: 'File', outputBinding: { glob: '*.haplotags*' }, doc: "This CSV/TSV file contains haplotag information for aligned reads. Note that while this contains the same information as the HP tag in the haplotagged output BAMs, generating those output BAMs is not required to generate this file." }

...
inputs:
  # Required
  bam: { type: 'File[]?', secondaryFiles: [{ pattern: ".bai", required: false}], inputBinding: { prefix: "--bam", position: 80 }, doc: "Input alignment file in BAM format" }
  vcf: { type: 'File[]?', inputBinding: { prefix: "--vcf", position: 82 }, doc: "Input variant file in VCF format" }
  reference: { type: 'File', inputBinding: { prefix: "--reference", position: 84 }, doc: "Reference FASTA file" }
  # Input/Output
  output_vcf: { type: 'string[]?', default: "deepvariant.hiphased.vcf.gz", inputBinding: { prefix: "--output-vcf", position: 84 }, doc: "Output phased variant file in VCF format" }
  output_bam: { type: 'string[]?', default: "hiphased.haplotagged.bam", inputBinding: { prefix: "--output-bam", position: 1 }, doc: "Output haplotagged alignment file in BAM format. [example: haplotagged.bam]" }
